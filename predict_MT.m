function [ mt ] = predict_MT(key1, key2 )
%Predicts the time in s to move from key1 to key2 in a 3x11 key grid, based on Fitts Law.
%   Input: key1, int
...
key1_y = mod(key1-1, 3)+1;
key1_x = mod(key1-1, 11)+1;
key2_y = mod(key2-1, 3)+1;
key2_x = mod(key2-1, 11)+1;
